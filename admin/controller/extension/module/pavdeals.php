...
class ControllerExtensionModulePavdeals extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if ($action == "module") {
				if (!isset($this->request->get['module_id'])) {
					...
					$this->response->redirect( $this->url->link('extension/module/pavdeals', 'token=' . $this->session->data['token'], 'SSL') );
				} else {

					$this->model_extension_module->editModule($this->request->get['module_id'], $this->request->post['pavdeals_module']);
					$this->response->redirect( $this->url->link('extension/module/pavdeals', 'token=' . $this->session->data['token'].'&module_id='.$this->request->get['module_id'], 'SSL') );
				}
			}
			if ($action == "config") {
		...
		$this->response->setOutput($this->load->view('extension/module/pavdeals.tpl', $this->mdata));
	...
	public function _dataModule(){
		...
		// action
		$this->mdata['delete'] = $this->url->link('extension/module/pavdeals/ndelete', 'token=' . $this->session->data['token'].$url, 'SSL');
		$this->mdata['action'] = $this->url->link('extension/module/pavdeals', 'token=' . $this->session->data['token'].$url, 'SSL');
	...
	public function module($extension){
		...
		foreach ($modules as $module) {
			$module_data[] = array(
				...
				'name'      => $module['name'],
				'edit'      => $this->url->link('extension/module/pavdeals', 'token=' . $this->session->data['token'] . '&module_id=' . $module['module_id'], 'SSL'),
			);
		}
		$ex[] = array(
			'name'      => $this->language->get("create_module"),
			'module'    => $module_data,
			'edit'      => $this->url->link('extension/module/pavdeals', 'token=' . $this->session->data['token'], 'SSL')
		);
		return $ex;
	}
	...
	public function _getStores() {
		...
		foreach ($stores as &$store) {
			...
			$store['option'] = $this->url->link('extension/module/pavdeals', $url.'&token=' . $this->session->data['token'], 'SSL');
		}
		return $stores;
	}
	...
	public function _language() {
		...
		$this->mdata['action'] = $this->url->link('extension/module/pavdeals', 'token=' . $this->session->data['token'], 'SSL');
	...
	public function _breadcrumbs() {
		...
		$this->mdata['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/module/pavdeals', 'token=' . $this->session->data['token'], 'SSL'),
			'separator' => ' :: '
		);
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/pavdeals')) {
